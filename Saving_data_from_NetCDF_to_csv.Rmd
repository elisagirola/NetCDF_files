...
datetime_converted_tz <- as.POSIXct(posix_converted, tz = tz) # Specify the time zone when creating the POSIXct object
```

Saving to a dataframe
```{r}
df <- data.frame(Time_days_since_18000101 = time, Time = datetime_converted_tz, Precipitation = precipitation, Soil_water_content = soil_water_content, Soil_temperature = soil_temperature, Air_temperature = air_temperature)
#plot(df$Air_temperature, df$Soil_temperature)
#plot(df$Precipitation, df$Soil_water_content)
plot(df$Time, df$Air_temperature)
```